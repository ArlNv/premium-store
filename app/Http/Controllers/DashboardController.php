...
class DashboardController extends Controller
{
    ...
    public function berandaPembeli()
    {
        ...
        $totalPembelian = \App\Models\Pembelian::where('user_id', $userId)->count();
        $last = \App\Models\Pembelian::where('user_id', $userId)->latest()->first();

        return view('pembeli.beranda', [
            'totalPembelian' => $totalPembelian,
            'lastStatus' => $last->status ?? null,
            'lastPaket' => $last->product->name ?? null,
        ]);
    }
